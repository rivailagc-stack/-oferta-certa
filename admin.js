...
async function selecionarImagens(event) {
  ...
  if (totalImagens > MAX_IMAGES) {
    mostrarMensagem("Máximo de 5 fotos por produto.", "error");
  ...
  for (const arquivo of novosArquivos) {
    if (!arquivo.type.startsWith("image/")) {
      mostrarMensagem("Escolha apenas arquivos de imagem.", "error");
      imageFilesInput.value = "";
      return;
    }
  ...
  try {
    for (const arquivo of novosArquivos) {
      ...
      arquivosSelecionados.push({
        ...
        preview: URL.createObjectURL(arquivoComprimido)
      });
    }
    ...
    console.error(error);
    mostrarMensagem("Não foi possível preparar as imagens.", "error");
  }
}
...
async function comprimirImagem(arquivo) {
  ...
  const blob = await new Promise((resolve, reject) => {
    canvas.toBlob(
      ...
      tipoSaida,
      tipoSaida === "image/png" ? undefined : 0.82
    );
  });
  ...
  try {
    ...
    const produto = {
      ...
      description:
        ...
        null,
      price: converterNumero(
        document.getElementById("price").value
      ),
      old_price: converterNumero(
...
function renderizarListaAdmin() {
  const termo =
    buscaAdmin.value.trim().toLowerCase();
