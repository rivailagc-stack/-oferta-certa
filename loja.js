...
function iniciarCheckout(){
  ...
  if(!freteSelecionado){
    alert("Escolha retirada no local ou calcule uma opção de entrega.");
    return;
  }

  if(!window.ofertaCertaCheckout?.open){
    alert("O pagamento seguro ainda está carregando. Aguarde alguns segundos e tente novamente.");
    return;
  }

  window.ofertaCertaCheckout.open({
    items,
    shipping:{
      ...
      service_name:Number(freteSelecionado?.service_id)===RETIRADA_SERVICE_ID
        ? "Retirada no local"
        : `${freteSelecionado?.company||"Transportadora"} — ${freteSelecionado?.name||"Entrega"}`,
      price:Number(freteSelecionado?.price||0),
      delivery_time:Number(freteSelecionado?.delivery_time||0),
      pickup:Number(freteSelecionado?.service_id)===RETIRADA_SERVICE_ID
    },
...
function calcularSubtotalCarrinho(){
  return obterCarrinho().reduce((total,item)=>{
    const produto=produtos.find(p=>String(p.id)===String(item.id));
    return total+(produto?Number(produto.price||0)*Number(item.quantity||1):0);
  },0);
}
...
async function divulgarProduto(p){
  const url=new URL(window.location.origin+window.location.pathname);url.searchParams.set("produto",p.id);url.hash="produtos";
...
window.ofertaCertaLoja = {
  clearCart(){
    localStorage.removeItem(CARRINHO_KEY);
    limparFreteSelecionado();
    atualizarCarrinho();
  },
  closeCart:fecharCarrinho,
  formatPrice:formatarPreco
};
