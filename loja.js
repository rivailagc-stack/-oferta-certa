...
function atualizarCarrinho(){
  ...
  const frete=completos.reduce((s,x)=>s+Number(x.produto.shipping_price||0),0);

  cartCount.textContent=quantidade;
  cartSubtotal.textContent=formatarPreco(subtotal);
  cartShipping.textContent=frete?formatarPreco(frete):"Grátis";
  cartTotal.textContent=formatarPreco(subtotal+frete);
  finalizarCompra.disabled=!completos.length;
