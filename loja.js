...
async function carregarProdutos(){
  ...
  produtos=data||[];preencherCategoriasAutomaticamente();esconderMensagem();renderizarProdutos();
}
...
function renderizarProdutos(){
  const termo=buscaInput.value.trim().toLowerCase(),categoria=categoriaSelect.value,ord=ordenacaoSelect.value;
  let filtrados=produtos.filter(p=>[p.title,p.description,p.category,p.marketplace].filter(Boolean).join(" ").toLowerCase().includes(termo)&&(!categoria||p.category===categoria));
...
function criarCardProduto(p){
  ...
  const cliques=Number(p.clicks||0),favorito=obterFavoritos().includes(String(p.id));
