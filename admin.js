...
const flashSaleCheckbox = document.getElementById("flash_sale");
const flashSaleField = document.querySelector(".flash-sale-field");
...
flashSaleCheckbox?.addEventListener("change", atualizarCampoOfertaRelampago);
...
async function salvarProduto(event) {
  ...
  try {
    ...
    const produto = {
      ...
      affiliate_link: document.getElementById("product_type").value === "affiliate"
        ? document.getElementById("affiliate_link").value.trim()
        : null,
      marketplace: document.getElementById("product_type").value === "affiliate"
        ? document.getElementById("marketplace").value
        : "Oferta Certa",
      stock: Number(document.getElementById("stock").value || 0),
      package_weight: converterNumero(document.getElementById("package_weight").value),
      package_length: Number(document.getElementById("package_length").value || 0) || null,
      package_width: Number(document.getElementById("package_width").value || 0) || null,
      package_height: Number(document.getElementById("package_height").value || 0) || null,
      colors: separarOpcoes(document.getElementById("colors")?.value),
      sizes: separarOpcoes(document.getElementById("sizes")?.value),
      featured: document.getElementById("featured").checked,
      flash_sale: document.getElementById("flash_sale").checked,
      flash_sale_end_at: document.getElementById("flash_sale").checked && document.getElementById("flash_sale_end_at").value
        ? new Date(document.getElementById("flash_sale_end_at").value).toISOString()
        : null,
  ...
  lista.innerHTML = filtrados.map(produto => `
    <article class="admin-product-card">
      ...
      <div class="admin-actions">
        ...
        <button type="button" class="icon-button" onclick="divulgarProdutoAdmin('${escaparAtributo(produto.id)}')">Divulgar</button>
...
function editarProduto(id) {
  ...
  document.getElementById("product_type").value = produto.product_type || "affiliate";
  document.getElementById("stock").value = Number(produto.stock || 0);
  document.getElementById("package_weight").value = numeroParaCampo(produto.package_weight);
  document.getElementById("package_length").value = produto.package_length || "";
  document.getElementById("package_width").value = produto.package_width || "";
  document.getElementById("package_height").value = produto.package_height || "";
  document.getElementById("colors").value = Array.isArray(produto.colors) ? produto.colors.join(", ") : (produto.colors || "");
  document.getElementById("sizes").value = Array.isArray(produto.sizes) ? produto.sizes.join(", ") : (produto.sizes || "");
  document.getElementById("flash_sale").checked = Boolean(produto.flash_sale);
  document.getElementById("flash_sale_end_at").value = produto.flash_sale_end_at ? dataHoraParaCampo(produto.flash_sale_end_at) : "";
  atualizarCamposTipoProduto();
  atualizarCampoOfertaRelampago();
...
function limparFormulario(apagarMensagem = true) {
  ...
  document.getElementById("product_type").value = "affiliate";
...
function atualizarCamposTipoProduto() {
  ...
  ["package_weight","package_length","package_width","package_height"].forEach(id => {
    document.getElementById(id).required = produtoProprio;
  });
}
...
window.duplicarProduto = id => {
  const produto = produtosAdmin.find(item => String(item.id) === String(id));
  if (!produto) return;

  document.getElementById("produtoId").value = "";
  document.getElementById("title").value = `${produto.title || "Produto"} - Cópia`;
  document.getElementById("description").value = produto.description || "";
  document.getElementById("price").value = numeroParaCampo(produto.price);
  ...
  document.getElementById("active").checked = !!produto.active;
  ...
  imagensExistentes = Array.isArray(produto.image_urls) && produto.image_urls.length
    ? [...produto.image_urls]
  ...
  tituloFormulario.textContent = "Duplicar produto";
  botaoCancelar.classList.remove("hidden");
  window.scrollTo({ top: 0, behavior: "smooth" });
};


function atualizarCampoOfertaRelampago() {
  if (!flashSaleField || !flashSaleCheckbox) return;
  flashSaleField.classList.toggle("hidden", !flashSaleCheckbox.checked);
}
...
window.divulgarProdutoAdmin = id => {
  const produto = produtosAdmin.find(item => String(item.id) === String(id));
  if (!produto) return;
  const url = new URL("index.html", window.location.href);
  url.searchParams.set("produto", produto.id);
  const texto = `🔥 ${produto.title}\n${formatarPreco(produto.price)}\nConfira aqui: ${url.href}`;
  if (navigator.share) navigator.share({ title: produto.title, text: texto, url: url.href }).catch(() => {});
  else navigator.clipboard.writeText(texto).then(() => alert("Texto e link copiados para divulgar!"));
};


/* V54 — ponte segura para o assistente de cadastro */
window.ofertaCertaAdminSmart = {
  getNewImages() {
    return arquivosSelecionados.map(item => ({
      file: item.file,
  ...
  replaceImages({ newImages = [], existingImages = [] } = {}) {
    arquivosSelecionados.forEach(item => {
      if (item.preview) URL.revokeObjectURL(item.preview);
    });

    imagensExistentes = Array.isArray(existingImages)
      ? existingImages.filter(Boolean).slice(0, MAX_IMAGES)
      : [];

    const limiteNovas = Math.max(
      0,
      MAX_IMAGES - imagensExistentes.length
    );

    arquivosSelecionados = (Array.isArray(newImages) ? newImages : [])
