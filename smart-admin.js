(() => {
  ...
  async function melhorarFotos() {
    ...
    api.showMessage(
      "Preparando fotos: enquadramento quadrado, fundo claro e ajuste de iluminação...",
      "info"
    );
    ...
    try {
      ...
      api.showMessage(
        "Fotos melhoradas. A primeira imagem continua sendo a capa.",
        "success"
      );
    } catch (error) {
      console.error(error);
  ...
  function tratarFoto(file) {
    return new Promise((resolve, reject) => {
      ...
      imagem.onload = () => {
        ...
        contexto.filter =
          "brightness(1.06) contrast(1.06) saturate(1.04)";

        const escala = Math.min(
          (tamanho - margem * 2) / imagem.naturalWidth,
          (tamanho - margem * 2) / imagem.naturalHeight
        );
