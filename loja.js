...
produtosContainer.addEventListener("click",async e=>{
  ...
  if(detalhes){e.preventDefault();abrirProduto(detalhes.dataset.detailsId);return}
...
function criarCardProduto(p){
  ...
  return `<article class="product-card">
    <div class="product-image-wrap">
      ...
      <button class="product-image-button" type="button" data-details-id="${escaparAtributo(p.id)}" aria-label="Ver detalhes de ${escaparAtributo(p.title||"produto")}">
    ...
    <div class="product-content">
      ...
      <button class="product-title-button" type="button" data-details-id="${escaparAtributo(p.id)}">
...
function abrirProduto(id){
  ...
  modalLoja.textContent=p.marketplace||"Loja online";
  modalCliques.textContent=`${Number(p.clicks||0)} clique${Number(p.clicks||0)===1?"":"s"}`;
  modalComprar.href=p.affiliate_link||"#";
  modalComprar.dataset.buyId=String(p.id);

  modalMiniaturas.innerHTML=imagens.map((url,index)=>`
    <button type="button" class="modal-thumbnail ${index===0?"active":""}" data-thumb-url="${escaparAtributo(url)}">
      <img src="${escaparAtributo(url)}" alt="Foto ${index+1}">
    </button>
  `).join("");

  modalMiniaturas.querySelectorAll("[data-thumb-url]").forEach(botao=>{
    botao.addEventListener("click",()=>{
      modalImagemPrincipal.src=botao.dataset.thumbUrl;
      modalMiniaturas.querySelectorAll(".modal-thumbnail").forEach(item=>item.classList.remove("active"));
