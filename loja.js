...
function adicionarAoCarrinho(id){
  ...
  const item=carrinho.find(x=>String(x.id)===String(id));
  if(item)item.quantity=Math.min(item.quantity+1,Number(produto.stock||0));
  else carrinho.push({id:String(id),quantity:1});
...
function removerDoCarrinho(id){
  salvarCarrinho(obterCarrinho().filter(x=>String(x.id)!==String(id)));
}
...
function atualizarCarrinho(){
  ...
  cartShipping.textContent=freteSelecionado ? formatarPreco(frete) : "Calcule o CEP";
  ...
  cartItems.innerHTML=completos.length?completos.map(x=>`
    <article class="cart-item">
      ...
      <div>
        ...
        <div class="cart-qty">
          <button type="button" data-cart-minus="${esc(x.id)}">−</button>
          <b>${x.quantity}</b>
          <button type="button" data-cart-plus="${esc(x.id)}">+</button>
          <button class="cart-remove" type="button" data-cart-remove="${esc(x.id)}">Remover</button>
        </div>
      </div>
    </article>
  ...
  try{
    const resposta=await fetch("/api/create-preference",{
      ...
      body:JSON.stringify({
        items,
        shipping:{
          postal_code:shippingCep.value.replace(/\D/g,""),
...
function renderizarOpcoesFrete(opcoes){
  shippingOptions.innerHTML=opcoes.map(opcao=>`
    <label class="shipping-option ${String(freteSelecionado?.service_id)===String(opcao.service_id)?"selected":""}">
      ...
      >
      <span>
        <strong>${texto(opcao.company)} — ${texto(opcao.name)}</strong>
        <small>${opcao.delivery_time} dia${Number(opcao.delivery_time)===1?"":"s"} úteis</small>
      </span>
      <b>${formatarPreco(opcao.price)}</b>
    </label>
  `).join("");

  shippingOptions.querySelectorAll('input[name="shipping_option"]').forEach(input=>{
    input.addEventListener("change",()=>{
      const opcao=opcoes.find(item=>String(item.service_id)===String(input.value));
      if(opcao)salvarFreteSelecionado(opcao);
    });
  });
}
